...
async function getMvpLoot(name, strict=true){
  ...
  const bestMatch = stringSimilarity.findBestMatch(name.replace(/[-_]/g,' '), this.mvpList).bestMatch;
  if( strict && bestMatch.rating < 0.5 ){
...
async function getMvpValue(name){
  ...
  const value = items.reduce((sum, item)=>{
    return sum + item.price * typeValue[item.type]
  },0)
